  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute -right-32 bottom-0 h-100 w-100 rounded-full bg-on-brand/3 blur-3xl" />
      ...
      <div className="relative mx-auto max-w-5xl">
        {/* ─── Badge ─── */}
        <div
          className={`mx-auto mb-6 w-fit rounded-full border border-on-brand/15 bg-on-brand/10 px-4 py-1.5 text-xs font-semibold text-on-brand/80 backdrop-blur-md transition-all duration-700 ${
            loaded ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
          }`}
        >
          <span className="mr-1.5 inline-block h-2 w-2 rounded-full bg-gold shadow-sm shadow-gold/50" />
          Now open at <span className="font-bold text-on-brand">FUOYE</span>
        </div>

        {/* ─── Heading ─── */}
        <h1
          className={`text-center font-display text-[clamp(2rem,7vw,3.75rem)] font-extrabold leading-[1.08] tracking-tight text-on-brand transition-all duration-700 delay-100 ${
        ...
        {/* ─── Subtitle ─── */}
        <p
          className={`mx-auto mt-4 max-w-xl text-center text-base leading-relaxed text-on-brand/65 transition-all duration-700 delay-200 md:text-lg ${
            loaded ? "translate-y-0 opacity-100" : "translate-y-6 opacity-0"
          }`}
        >
        ...
        <div className="mx-auto mt-10 grid max-w-2xl grid-cols-2 gap-3 md:gap-4">
          {BENEFITS.map((ben, i) => (
            <div
              key={ben.label}
              className={`group rounded-2xl border border-on-brand/10 bg-on-brand/6 px-4 py-4 backdrop-blur-sm transition-all duration-500 hover:border-on-brand/20 hover:bg-on-brand/10 hover:shadow-lg ${
            ...
            >
              ...
              <p className="text-sm font-bold text-on-brand">{ben.label}</p>
              <p className="mt-0.5 text-xs text-on-brand/50">{ben.desc}</p>
            </div>
          ))}
        </div>
        ...
        >
          ...
          <button
            type="button"
            className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl border-2 border-on-brand/30 bg-on-brand/5 px-8 py-3.5 text-[15px] font-semibold text-on-brand/90 backdrop-blur-sm transition-all duration-200 hover:bg-on-brand/10 hover:text-on-brand active:scale-[0.97]"
          >
            Learn More
          </button>
        </div>
        ...
        {/* ─── Scroll indicator ─── */}
        <div className="mx-auto mt-14 flex animate-bounce flex-col items-center gap-1 text-on-brand/30">
