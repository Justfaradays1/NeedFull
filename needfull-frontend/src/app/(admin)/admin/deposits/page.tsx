    ...
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <div className="mx-4 mt-3 space-y-3">
        {loading ? (
          ...
          deposits.map((d) => (
            <div key={d.id} className="rounded-2xl border border-card-border bg-surface p-4 shadow-card transition-shadow duration-200 hover:shadow-lifted active:scale-[0.99]">
      ...
      {enlargeUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setEnlargeUrl(null)}>
          <div className="relative max-h-[80vh] max-w-full">
            <button type="button" onClick={() => setEnlargeUrl(null)} className="absolute -right-3 -top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-surface shadow-lg border border-card-border">
