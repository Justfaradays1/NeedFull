...
function RunnerCard({ runner, onTap }: RunnerCardProps) {
  ...
  return (
    <button
      type="button"
      onClick={onTap}
      className="tap-target flex w-35 shrink-0 flex-col items-center rounded-2xl border border-card-border bg-surface p-4 text-center shadow-card transition-shadow duration-200 active:scale-[0.99] hover:border-brand/20"
    >
      ...
      <p className="mt-2 w-full truncate text-sm font-bold text-gray-900">
        {runner.fullName}
      </p>
      {topSkill && (
        <p className="w-full truncate text-[10px] text-gray-500">{topSkill}</p>
      )}
      ...
      <div className="mt-3">
        ...
        {credits >= 500 && (
          <p className="mt-1 text-xs font-bold text-green-600">
            You&apos;ve reached 500 credits! 🎉
          </p>
        )}
      </div>
      <div className="mt-3 space-y-1 text-[11px] text-gray-600">
        <p className="flex items-center gap-1.5">
          <Star className="h-3 w-3 text-gold" /> Complete tasks — earn credits
          per task
        </p>
        <p className="flex items-center gap-1.5">
    ...
    <div className="min-h-screen page-shell pb-8">
      ...
      <div className="glass-dark px-4 pb-4 pt-3">
        ...
        <p className="mt-0.5 text-xs text-white/70 sm:text-sm">
          Discover tasks, runners, and more
        </p>
      </div>
          ...
          {loadingRunners ? (
            ...
            <div className="rounded-xl bg-gray-50 p-6 text-center">
              ...
              <p className="mt-2 text-sm font-medium text-gray-600">
                No runners available near you right now
              </p>
            ...
            {[
              {
                ...
                desc: "Design, typing, editing",
                icon: <ShoppingBag className="h-6 w-6 text-white/60" />,
              },
              {
              ...
              {
                title: "Lending Circle",
                desc: "Peer micro-loans for verified students",
                icon: <CircleDollarSign className="h-6 w-6 text-white/60" />,
              },
            ].map((s) => (
              <div
                key={s.title}
                className="relative flex flex-col items-center rounded-2xl border border-card-border bg-surface p-5 text-center shadow-card"
              >
                <span className="absolute right-2 top-2 inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-[9px] font-bold text-gray-500">
                  Coming soon
                </span>
