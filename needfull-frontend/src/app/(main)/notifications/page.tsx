...
function notifNavigate(n: Notification, router: ReturnType<typeof useRouter>) {
  ...
  if (
    ...
    n.type.startsWith("manual_transfer_") ||
    n.type.startsWith("transfer_") ||
    n.type === "nuban"
  ) {
    router.push("/wallet");
    return;
...
export default function NotificationsPage() {
  ...
  const handleTouchStart = useCallback((e: TouchEvent) => {
    const id = (e.currentTarget as HTMLElement).getAttribute("data-notif-id");
    if (id) longPressTimer.current = setTimeout(() => setMenuId(id), 500);
  }, []);
      ...
      <div className="glass-dark sticky top-0 z-30 flex items-center justify-between px-4 py-3">
        <div>
          ...
          <p className="text-xs text-white/70 sm:text-sm">
            Stay updated on your tasks and activity
          </p>
        </div>
        {unreadCount > 0 && (
          <button
                    ...
                    <div key={n.id} className="relative">
                      ...
                      >
                        <div className="flex gap-3">
                          <div
                            className={`relative mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${meta.bg}`}
                          >
                            {meta.icon}
                            {showUnread && (
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-2">
                              <p
                                className={`text-sm ${showUnread ? "font-bold text-gray-900" : "font-medium text-gray-500"}`}
                              >
                                {n.title}
                              </p>
                      ...
                      {menuId === n.id && (
                        <>
                          ...
                          />
                          <div className="absolute right-2 top-2 z-50 rounded-xl border border-card-border bg-surface py-1 shadow-lg min-w-35">
                            {showUnread && (
                              <button
                                type="button"
