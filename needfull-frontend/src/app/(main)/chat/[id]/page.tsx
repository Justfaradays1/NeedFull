...
import { ChatDetailSkeleton } from "@/components/ui/skeletons/ChatDetailSkeleton";
    ...
    <div className={`flex ${isMine ? "justify-end" : "justify-start"} px-4`}>
      ...
      >
        ...
        >
          ...
          {isMine && (
            <CheckCheck
              className={`h-3 w-3 ${msg.isRead ? "text-blue-300" : ""}`}
            />
          )}
        </div>
      </div>
...
export default function ChatThreadPage() {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(
      () => emit("stop:typing", { conversationId: convId, userId: user.id }),
      2000,
    );
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      }>(`/chat/conversations/${convId}/messages`, { content: text });
      if (res.success) {
        const msg = res.data;
        ...
        <div className="relative">
          ...
          {menuOpen && (
            <>
              ...
              />
              <div className="absolute right-0 top-full z-20 mt-1 rounded-xl border border-card-border bg-surface py-1 shadow-lg min-w-40">
                <button
                  type="button"
                  onClick={() => {
                    setMenuOpen(false);
