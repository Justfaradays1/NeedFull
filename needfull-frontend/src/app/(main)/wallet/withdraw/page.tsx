...
export default function WithdrawPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await get<{ success: boolean; data: Withdrawal[] }>(
        "/wallet/withdraw?perPage=20",
      );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (res.success) {
        ...
        celebration.showForAction("poster", "withdrawal_submitted", {
          ...
          <div className="mx-4 -mt-4 sm:mx-6">
            <div className="rounded-2xl border border-card-border bg-surface p-5 text-center shadow-card transition-shadow duration-200 hover:shadow-lifted active:scale-[0.99]">
              ...
              <p className="mt-4 text-lg font-bold text-gray-900">
                {formatCurrency(Math.round(withdrawalResult.amount * 100))}
              </p>
              <p className="mt-1 text-sm text-gray-600">
                will be sent to <strong>{withdrawalResult.bankName}</strong>{" "}
                {maskAccount(withdrawalResult.accountNumber)}
              </p>
        ...
        <div className="min-h-screen page-shell safe-all">
          ...
          <div className="mx-4 -mt-4 space-y-4 pb-8 sm:mx-6">
            ...
            <ConfirmationDialog
              ...
              message={`You are withdrawing ${formatCurrency(Math.round(amountNum * 100))} to ${selectedBank?.name} (${accountNumber}). A ₦50 fee applies. This action cannot be undone.`}
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  {bankDropdownOpen && (
                    <div className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border border-card-border bg-surface shadow-lg">
                ...
                {errors.bank && (
                  <p className="text-xs text-danger">{errors.bank}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.accountNumber && (
                  <p className="text-xs text-danger">{errors.accountNumber}</p>
                )}
              </div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder={
                      accountNameLoading
                        ? "Verifying..."
                        : "Auto-fills from account number"
                    }
                    value={accountName}
                    onChange={(e) => {
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-gray-900">
                          {formatCurrency(Math.round(w.amount.naira * 100))}
                        </p>
                        <StatusBadge status={w.status} />
                      </div>
                      <p className="text-xs text-gray-500">
                        {w.bankName || "Bank"} {maskAccount(w.accountNumber)}{" "}
                        &middot; {fmtDate(w.createdAt)}
                      </p>
                      {w.failureReason && (
